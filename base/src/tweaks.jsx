...
function useTweaks() {
  const [state, setState] = React.useState(() => {
    try {
      return (
        JSON.parse(localStorage.getItem("tweaks") || "null") || TWEAK_DEFAULTS
      )
    } catch {
...
function TweaksPanel({ tweaks }) {
  ...
  return (
    ...
    >
        ...
        <div style={{ display: "grid", gap: 6 }}>
          {accentPairs.map(([c1, c2, name]) => (
            <div
              ...
              style={{
                padding: 8,
                border: `1px solid ${state.accentExpense === c1 ? T.ink : T.rule}`,
                borderRadius: 2,
                cursor: "pointer",
                display: "flex",
                alignItems: "center",
      ...
      <div
        style={{ fontSize: 10, color: T.muted, marginTop: 10, lineHeight: 1.5 }}
      >
        Reload the page after changing accents to re-render all color-derived
        chips.
      </div>
    </div>
  )
